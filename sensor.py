...
async def async_setup_entry(
    ...
    # Créer le dashboard automatiquement
    await create_default_dashboard(hass, config_entry)

    # Start websocket connection
    asyncio.create_task(websocket_to_mqtt(hass, config, config_entry))

async def create_default_dashboard(hass: HomeAssistant, config_entry: ConfigEntry) -> None:
    """Create the default dashboard for Storcube."""
    dashboard_config = {
                        ...
                        "cards": [
                            {
                                ...
                                "title": "Flux d'Énergie Storcube",
                                "entities": {
                                    "grid": [
                                        {
                                            "entity": f"sensor.{config_entry.entry_id}_output_power",
                                            "name": "Onduleur",
                                            "icon": "mdi:power-plug"
                                        }
                                    ],
                                    "solar": [
                                        {
                                            "entity": f"sensor.{config_entry.entry_id}_solar_power",
                                            "name": "Solaire 1",
                                            "icon": "mdi:solar-power"
                                        },
                                        {
                                            "entity": f"sensor.{config_entry.entry_id}_solar_power_2",
                                            "name": "Solaire 2",
                                            "icon": "mdi:solar-power"
                                        }
                                    ],
                                    "battery": [
                                        {
                                            "entity": f"sensor.{config_entry.entry_id}_battery_level",
                                            "name": "Batterie",
                                            "icon": "mdi:battery"
                                        }
                                    ]
                            ...
                            },
                            {
                                ...
                                "cards": [
                                    ...
                                    },
                                    {
                                        "type": "gauge",
                                        "name": "Santé Batterie",
                                        "entity": f"sensor.{config_entry.entry_id}_battery_health",
                                        "min": 0,
                                        "max": 100,
                                        "severity": {
                                            "green": 80,
                                ...
                                "cards": [
                                    {
                                        "type": "entity",
                                        "name": "Température",
                                        "entity": f"sensor.{config_entry.entry_id}_battery_temperature",
                                        "icon": "mdi:thermometer"
                                    },
                                    {
                                        "type": "entity",
                                        "name": "État",
                                        "entity": f"sensor.{config_entry.entry_id}_battery_status",
    ...
    dashboard_id = f"storcube_{config_entry.entry_id}"
    if dashboard_id not in hass.data.get("lovelace", {}).get("dashboards", {}):
        try:
            ...
            _LOGGER.info("Dashboard Storcube créé avec succès")
        except Exception as e:
            _LOGGER.error("Erreur lors de la création du dashboard: %s", str(e))
